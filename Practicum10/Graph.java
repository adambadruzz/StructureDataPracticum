...
public class Graph {
    ...
    public Graph() {
        ...
        for (int i = 0; i < MAX_VERTS; i++) { //set adjacency
          for (int j = 0; j < MAX_VERTS; j++) { //matriks to 0
                adjMat[i][j] = 0;
          }
        }
    ...
    public void Connectivity(){
        System.out.println("-------Connectivity Table-------");//mencetak
        for(int row = 0;row < nVerts; row++){//looping row
            System.out.print(vertexList[row]+" ");
            for(int col = 0;col < nVerts; col++){//looping colom
                if(adjMat[row][col]==1){
                    System.out.print(vertexList[col].label+" ");
                    getAdjUnvisitedVertex(col);
                }
            }
            resetFlags();//reset vertex
            System.out.println("");
        }
    }
    
    public void topo(){
        int orig_nVerts = nVerts;//berapa bnyk verts
        while(nVerts > 0){//while vertices remain
            int currentVertex = noSuccessors(); //get a vertex with noSucessors
            if(currentVertex == -1){
                System.out.println("Error: Graph has cycles");
                return;
            }
            sortArray[nVerts - 1] = vertexList[currentVertex].label;
